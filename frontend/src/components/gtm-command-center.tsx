...
type Step = { action: "research"; target_index: number; dependencies: number[]; rationale: string; expected_output: string; side_effect: "read_only"; approval_required: true };
...
type CycleDetail = Cycle & { steps: { position: number; status: string; output: { research_job_id?: string } | null }[]; commands: { id: string; status: string; attempts: number; error_code: string | null }[]; events: { kind: string; at: string }[] };
...
export function GtmCommandCenter() {
  ...
  async function approve() {
    ...
    setMessage("Plan approved and queued for the background worker. No outbound communication is authorized by this research plan.");
  }
  return <section className="space-y-4 rounded-lg border p-5" aria-labelledby="gtm-center">
    ...
    <p className="text-sm text-muted-foreground">Give your GTM employee a goal, review its plan, then approve execution. Research is the currently available action; paid services and outbound communication are blocked.</p>
    ...
    {plan && <article className="space-y-3 rounded border p-4">
      ...
      <p className="text-xs text-muted-foreground">Created by {plan.author_method.startsWith("ollama:") ? "local AI" : plan.author_method === "explicit_research_template" ? "research template" : "customer revision"}. Paid API budget: $0.</p>
      <label className="block text-sm">Objective<textarea className={style} disabled={busy || plan.status === "approved"} value={plan.document.plan.objective} onChange={e => edit({ ...plan.document.plan, objective: e.target.value })} /></label>
      ...
      <ol className="space-y-3">{plan.document.plan.steps.map((s,index) => <li key={index} className="rounded border p-3"><p className="font-medium">{index + 1}. Research {companies.data?.find(c => c.id === plan.document.targets[s.target_index]?.company_id)?.name || "target account"}</p><p className="text-sm">{s.rationale}</p><p className="text-sm">Expected: {s.expected_output}</p><p className="text-xs">Read-only · dependencies: {s.dependencies.length ? s.dependencies.map(d => d + 1).join(", ") : "none"}</p></li>)}</ol>
