...
type Step = { action: "research" | "outreach_send"; message_id?: string | null; target_index: number; dependencies: number[]; rationale: string; expected_output: string; side_effect: "read_only" | "outbound"; approval_required: true };
...
type CycleDetail = Cycle & { steps: { position: number; status: string; output: { research_job_id?: string; message_id?: string; provider_message_id?: string } | null }[]; commands: { id: string; status: string; attempts: number; error_code: string | null }[]; events: { kind: string; at: string }[] };
...
export function GtmCommandCenter() {
  ...
  async function approve() {
    ...
    setMessage(plan.document.plan.steps.some(s => s.action === "outreach_send") ? "Reviewed outreach queued for the background worker. Live providers remain disabled." : "Research plan approved and queued for the background worker.");
  }
  return <section className="space-y-4 rounded-lg border p-5" aria-labelledby="gtm-center">
    ...
    {plan && <article className="space-y-3 rounded border p-4">
      ...
      <p className="text-xs text-muted-foreground">Created by {plan.author_method.startsWith("ollama:") ? "local AI" : plan.author_method === "explicit_research_template" ? "research template" : plan.author_method === "reviewed_outreach" ? "reviewed outreach" : "customer revision"}. Paid API budget: $0.</p>
      <label className="block text-sm">Objective<textarea className={style} disabled={busy || plan.status === "approved" || plan.author_method === "reviewed_outreach"} value={plan.document.plan.objective} onChange={e => edit({ ...plan.document.plan, objective: e.target.value })} /></label>
      ...
      <ol className="space-y-3">{plan.document.plan.steps.map((s,index) => <li key={index} className="rounded border p-3"><p className="font-medium">{index + 1}. {s.action === "outreach_send" ? "Send approved outreach to" : "Research"} {companies.data?.find(c => c.id === plan.document.targets[s.target_index]?.company_id)?.name || "target account"}</p><p className="text-sm">{s.rationale}</p><p className="text-sm">Expected: {s.expected_output}</p><p className="text-xs">{s.side_effect === "outbound" ? "Outbound: separate message approval required" : "Read-only"} · dependencies: {s.dependencies.length ? s.dependencies.map(d => d + 1).join(", ") : "none"}</p></li>)}</ol>
      ...
      {plan.status === "draft" && <><Button disabled={!dirty || busy || plan.author_method === "reviewed_outreach"} variant="outline" onClick={() => void run(async () => selectPlan(await apiFetch<Plan>(`/api/v1/gtm/plans/${plan.id}/revisions`, { method: "POST", body: JSON.stringify(plan.document.plan) })))}>Save revised plan</Button><label className="flex items-center gap-2 text-sm"><input type="checkbox" disabled={dirty || busy} checked={reviewed} onChange={e => setReviewed(e.target.checked)} />I reviewed this saved plan, targets, constraints and risks.</label><Button disabled={!reviewed || dirty || busy} onClick={() => void run(approve)}>Approve and queue plan</Button></>}
      <Button variant="outline" disabled={busy || dirty || plan.author_method === "reviewed_outreach"} onClick={() => void run(async () => selectPlan(await apiFetch<Plan>(`/api/v1/gtm/goals/${plan.goal_id}/plans`, { method: "POST", body: JSON.stringify({ mode: "local_ai" }) })))}>Generate a new plan version with local AI</Button>
    </article>}
