...
export function AccountResearch({ companyId }: { companyId: string }) {
  ...
  async function run() {
    ...
    try {
      const job = await apiFetch<Job>("/api/v1/research/jobs", { method: "POST", body: JSON.stringify({ company_id: companyId, brain_version_id: version, source_urls: urls.split("\n").map(s => s.trim()).filter(Boolean) }) });
      setSelected(job.id);
      await apiFetch(`/api/v1/research/jobs/${job.id}/run`, { method: "POST" });
  ...
  return <section className="mt-5 space-y-4 border-t pt-4" aria-labelledby="research-heading">
    ...
    <Button disabled={busy || !version || !urls.trim()} onClick={() => void run()}>{busy ? "Capturing and analyzing sources…" : "Research these sources"}</Button>
